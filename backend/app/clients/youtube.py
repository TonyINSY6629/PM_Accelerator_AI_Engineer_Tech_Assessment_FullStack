...
def search_walking_tour(city_name: str, country: str, max_results: int = 3) -> list[dict]:
    cache_key = f"{city_name}, {country}" # --------------------------------identifies the place
    search_query = f"{city_name} 4K walking tour" # ------------------------what YouTube is actually asked
...
if __name__ == "__main__":
    for each_video in search_walking_tour("New York", "US"):
        print(each_video)
    print("second call hits the cache:", search_walking_tour("New York", "US") is search_walking_tour("New York", "US"))